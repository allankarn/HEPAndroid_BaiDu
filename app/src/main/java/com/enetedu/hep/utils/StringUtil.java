...
/**
 * 作者：jiahuibin on 2016/6/11 14:34
 * 邮箱：zeus_jhb@qq.com
 */
public abstract class StringUtil {
    ...
    public static String processBackSlashForJSON(String input) {
         ...
         * 过滤[\"],[\\],[\/],[\b],[\f],[\n],[\r],[\t]这几种
         */
        Pattern pattern = Pattern.compile("\\\\[^\"v\b\f\n\r\t/\\\\]");
...
//                    }
    ...
    public static String left0(String str, int len) {
        ...
        if (strLen == 0) {//请求为C时，全补空格，长度多少补多少
            for (int i = 0; i < len; i++) {
...
//    public static String addFourSpace(String one,String two,String three,String four, int len) {
//        int length = getLength(one)+getLength(two)+getLength(three)+getLength(four);
//        if (length >= len) {
//            return ;
//        } else {
//            int count = len - length;
...
//    }
  ...
  /**
     ...
     * @return
     */
    public static String cardNumConfuse(String value , int headLength , int endLength){
        value = value.trim();
        if (!TextUtils.isEmpty(value)) {
            int length = value.length();
            if (length> (headLength + endLength)) {
                int confuseNum = value.length() - (headLength + endLength);
                String head = value.substring(0 , headLength);
                String end = value.substring(headLength + confuseNum);
                StringBuilder builder = new StringBuilder();
                for (int i = 0; i < confuseNum; i++) {
                    builder.append("*");
                }
                return head + builder + end;
            }else{
                return value;
            }
    ...
    /**
     * 由小数字符串转成，没点的字符串(只取左边)
     *
     * @param money
     * @return
     */
    public static String moneyToStringNoLast(String money) {
        try {
            if (money == null) {
                return "";
            }
     ...
     * @param width
     */
    public static String[] splitWithLength(String str,int width) {
        ...
        if (chars.length<width)
        {
            ...
            for (int i = 0; i < chars.length; i++) {
                childStr+=chars[i];
                if (i!=0 && i%width==0||i == chars.length-1)
                {
                    array.add(childStr);
                    childStr ="";
                }
    ...
    public static String getFormatOilMoney(String domain48){
        if (domain48.length()<3)
        {
            return "0";
        }
        String balanceAmouont="";
        String amount_sign = domain48.substring(3,4);
        if ("C".equals(amount_sign)) {
        } else if("D".equals(amount_sign)){
            balanceAmouont += " - ";
        }
        balanceAmouont += stringToMoney(domain48.substring(4).trim());
     ...
     * @return
     */
    public static String addSpaceToRight(String str, int length) {
        int strLen;
        ...
        if (strLen == length) {
            return str;
        } else if (getLength(str) < length) {
            int temp = length - length1;
            String tem = "";
     ...
     * @return
     */
    public static String addLeftSpaceNextLine(String s, int len) {

        int length = getLength(s);
        if (length >= len) {
            String top = s.substring(0 , len);
            String bottom = s.substring(len);
            StringBuffer stringBuffer = new StringBuffer(top);
            stringBuffer.append("\n").append(bottom);
    ...
    /**
     * 将 带小数的字符串转成，没点的字符串 定长12位 长度不够 左补零
     *
     * @param money
     * @return
     */
    public static String moneyTo12String(String money) {
        if (money == null) {
            return "000000000000";
        }
        if (money.contains(".")) {
            ...
            String left_num = arr[0];
            String right_num = arr[1];

            while ( (left_num.length()+right_num.length()) < 12 ){
                left_num = "0" + left_num ;
            }

            return left_num + arr[1];
        } else {
     ...
     * @return
     */
    public static String stringToMoney(String content) {
        if (content == null || content.length() == 0 || content.equals(" ")) {
            ...
            try {
                content = String.format("%.2f", Double.parseDouble(content) / 100);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public static String stringToLitre(String content) {
        if (content == null || content.length() == 0 || content.equals(" ")) {
            content = "";
        } else if (content.contains(".")){
            content = content;
            ...
            try {
                content = String.format("%.3f", Double.parseDouble(content) / 1000);
            } catch (Exception e) {
                e.printStackTrace();
            }
     ...
     * @return
     */
    public static String getOilCardInteger(String value){
        String temp = value.substring(4);
        String index = value.substring(3, 4);
        temp = stringToMoney(temp);
        String result = "";
        if (index.equals("C")){//正值
            result = temp;
        }else {
            result = "-" + temp;
        }
     ...
     * @return
     */
    public static String getOilCardBalance(String value){
...
//        }
    ...
    public static String addSpaceToLeft(String value , int targetLength){
        if (value.contains(".")){
            value = value.replace("." , "");
        }
        int valueLength = getLength(value);
        StringBuffer stringBuffer = new StringBuffer(value);
        if (valueLength > targetLength){//大于  截取前几位
            ...
            stringBuffer = new StringBuffer(value.substring(0 , resultIndex + 1));
            //避免少一位
            while (getLength(stringBuffer.toString()) < targetLength){
