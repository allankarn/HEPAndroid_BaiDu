...
public class RespFinishedList extends RespBase {
    private ArrayList<Course3ViewListEntity> list = new ArrayList<>();
                ...
                if (jsonArray != null && jsonArray.length() > 0) {
                    for (int i = 0; i < jsonArray.length(); i++) {
                        try {
                            ...
                            String numbers = jsonObject.optString("numbers");
                            ...
                            course3ViewListEntity = new Course3ViewListEntity(id, name, courseType, courseStatus, startTime, endTime, createTime, finishTime,
                                    is_payoffed, numbers, is_video_finished, credit_remain_event, is_work_finished, statusName, courehour, is_new, is_postpone, operate, courseLists);

                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
                        list.add(course3ViewListEntity);
                    }
                }
            }
        }
    ...
    public String toString() {
        return "RespFinishedList{" +
                "list=" + list +
                ", course3ViewListEntity=" + course3ViewListEntity +
                '}';
    }
}
